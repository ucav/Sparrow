...
- `computer`: focused computer-use primitive for `screenshot`, `click`, and
  `type`. It is classified as `Exec` so the autonomy gate treats UI-driving
  actions more strictly than passive web reads.
